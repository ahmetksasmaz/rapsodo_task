...
class MetadataPrinter:
    ...
    def process(self, message):
        ...
        image[int(metadata.ball_target_bbox.y + metadata.ball_target_bbox.h / 2)][int(metadata.ball_target_bbox.x + metadata.ball_target_bbox.w / 2)][:] = [0,255,255]
        cv.rectangle(image, (int(metadata.ball_target_bbox.x), int(metadata.ball_target_bbox.y)), (int(metadata.ball_target_bbox.x + metadata.ball_target_bbox.w), int(metadata.ball_target_bbox.y + metadata.ball_target_bbox.h)), (0,255,0), 2)
        
        cv.imwrite('output_images_' + str(message_id) + '.png', image)
        
        print("#######################################")
        print("Start of Message ID : ", "\t>>>>>>>>>", str(message_id), "<<<<<<<<<")
        print("Ball Bounding Box [x,y,w,h] : ", "[", str(metadata.ball_target_bbox.x), ",", str(metadata.ball_target_bbox.y), ",", str(metadata.ball_target_bbox.w), ",", str(metadata.ball_target_bbox.h), "]")
