...
class Plotter:
    ...
    def __init__(self, output_file_name):
        ...
        for line in self.output_file.readlines():
            ...
            self.data["ball_avg_3d_speed_vector_x"].append(float(parts[1]))
            self.data["ball_avg_3d_speed_vector_y"].append(float(parts[2]))
            self.data["ball_avg_3d_speed_vector_z"].append(float(parts[3]))
            self.data["gt_ball_bbox_iou"].append(float(parts[4]))
            self.data["gt_ball_2d_points_symmetric_difference_error"].append(float(parts[5]))
            self.data["gt_ball_2d_center_error"].append(float(parts[6]))
    def plot(self):
